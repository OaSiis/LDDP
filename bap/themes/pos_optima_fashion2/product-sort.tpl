...
<form id="productsSortForm{if isset($paginationId)}_{$paginationId}{/if}" action="{$request|escape:'html':'UTF-8'}" class="productsSortForm">
	<div class="select selector1">
		...
		<select id="selectProductSort{if isset($paginationId)}_{$paginationId}{/if}" class="selectProductSort form-control">
			<option value="{$orderbydefault|escape:'html':'UTF-8'}:{$orderwaydefault|escape:'html':'UTF-8'}" {if $orderby eq $orderbydefault}selected="selected"{/if}>-- Select option --</option>
			{if !$PS_CATALOG_MODE}
...
<ul class="display hidden-xs">
    ...
    <li id="grid"><a rel="nofollow" href="#" title="{l s='Grid'}">{l s='Grid'}</a></li>
    <li id="list"><a rel="nofollow" href="#" title="{l s='List'}">{l s='List'}</a></li>
</ul>
{/if}
